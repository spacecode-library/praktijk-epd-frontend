...
const TherapistContractDetail: React.FC = () => {
  ...
  const loadContract = async () => {
    ...
    setLoading(true);
    try {
      const response = await realApiService.contracts.therapist.get(id);
      if (response.success && response.data) {
        setContract(response.data);
      } else {
        error('Failed to load contract');
        navigate('/therapist/contracts');
      }
    } catch (err: any) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          <PremiumButton
            variant="outline"
            icon={ArrowLeftIcon}
            onClick={() => navigate('/therapist/contracts')}
          >
            Back to Contracts
          </PremiumButton>
          <div>
            ...
            <p className="text-sm text-gray-500">
              Contract with {contract.client_name}
            </p>
          </div>
        </div>
      </div>
      ...
      <PremiumCard className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Client Information */}
          <div>
            <h3 className="text-sm font-medium text-gray-500 uppercase mb-3">Client Information</h3>
      ...
      {/* Info Box */}
      <PremiumCard className="p-4 bg-blue-50 border-blue-100">
        <div className="flex items-start space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-blue-700 mt-1">
              For any changes to this contract, please contact your administrator.
            </p>
          </div>
        </div>
      </PremiumCard>
...
export default TherapistContractDetail;
